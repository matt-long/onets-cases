module ecosys_forcing_mod
  ...
  integer(int_kind)   :: alk_forcing_shr_stream_year_align   ! align with this model year
  character(char_len) :: alk_forcing_shr_stream_file         ! file with domain and input data
  real(r8)            :: alk_forcing_shr_stream_scale_factor ! unit conversion factor


  logical(log_kind)   :: lfe_fert_forcing_apply_file_flux
  integer(int_kind)   :: fe_fert_forcing_shr_stream_year_first   ! first year in stream to use
  integer(int_kind)   :: fe_fert_forcing_shr_stream_year_last    ! last year in stream to use
  integer(int_kind)   :: fe_fert_forcing_shr_stream_year_align   ! align with this model year
  character(char_len) :: fe_fert_forcing_shr_stream_file         ! file with domain and input data
  real(r8)            :: fe_fert_forcing_shr_stream_scale_factor ! unit conversion factor
...
contains
  ...
  subroutine ecosys_forcing_init(ciso_on, land_mask,                     &
    ...
    namelist /ecosys_forcing_data_nml/                                        &
         ...
         alk_forcing_shr_stream_year_last,                                    &
         alk_forcing_shr_stream_year_align,                                   &
         alk_forcing_shr_stream_file,                                         &
         alk_forcing_shr_stream_scale_factor,                                 &
         lfe_fert_forcing_apply_file_flux,                                    & 
         fe_fert_forcing_shr_stream_year_first,                               &
         fe_fert_forcing_shr_stream_year_last,                                & 
         fe_fert_forcing_shr_stream_year_align,                               &
         fe_fert_forcing_shr_stream_file,                                     & 
         fe_fert_forcing_shr_stream_scale_factor,                             &         
    ...
    fe_fert_forcing_shr_stream_file = 'none'
    fe_fert_forcing_shr_stream_scale_factor = 1.0e8_r8 ! convert from mol/m^2/s to nmol/cm^2/s    
    ...
    do n=1,size(surface_flux_forcings)
      ...
      select case (trim(marbl_req_surface_flux_forcings(n)%metadata%varname))
        ...
        case ('Fe Fertilization Flux')
            if (lfe_fert_forcing_apply_file_flux) then
                call surface_flux_forcings(n)%add_forcing_field(field_source='shr_stream',       &
                     strdata_inputlist_ptr=surface_strdata_inputlist_ptr,                        &
                     marbl_varname=marbl_varname,                                                &
                     field_units=units,                                                          &

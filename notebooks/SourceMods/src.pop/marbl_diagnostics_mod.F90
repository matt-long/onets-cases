...
contains
  ...
  subroutine marbl_diagnostics_init(  &
    ...
    associate(                                    &
      ...
      lname    = 'Flux of Fe from forcing data'
      sname    = 'FE_FERT_FLUX'
      units    = 'nmol/cm^2/s'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%FE_FERT_FLUX, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if           
  ...
  subroutine marbl_diagnostics_surface_flux_compute( &
    ...
    associate(                                                                                  &
         ...
         fe_fert_flux      => surface_flux_forcings(surface_flux_forcing_ind%fe_fert_flux_id)%field_0d, & 
    ...
    !-----------------------------------------------------------------------
    !  calculate fe_fert flux
    !-----------------------------------------------------------------------

    diags(ind_diag%FE_FERT_FLUX)%field_2d(:) = fe_fert_flux
